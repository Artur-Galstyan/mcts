...
def backpropagate(tree: Tree, leaf_index: int):
...
def inference_worker(n_workers, step_queue, step_fn, result_queue):
    while True:
        try:
            batch_inputs = []
            batch_size = min(n_workers, step_queue.qsize())
            if batch_size == 0:
                continue

            for _ in range(batch_size):
                if not step_queue.empty():
                    batch_inputs.append(step_queue.get(timeout=0.1))

            if batch_inputs:
                embeddings = [inp.embedding for inp in batch_inputs]
                actions = np.array([inp.action for inp in batch_inputs])

                batched_input = BatchedStepFnInput(
                    embeddings=embeddings, actions=actions
                )
...
def run_simulation(args):
    ...
    if tree.children_indices[parent_index, action] == UNVISITED:
        ...
        step_queue.put(step_input)

        step_result = result_queue.get()
...
class MCTS:
    ...
    def search(
        ...
        inference_thread = multiprocessing.Process(
            ...
            args=(n_workers, step_queue, step_fn, result_queue),
            daemon=True,
        )
        inference_thread.start()

        with multiprocessing.pool.ThreadPool(processes=n_workers + 1) as pool:
            simulation_args = (
                tree,
                step_queue,
                result_queue,
